package com.jcnovelashd.novelasgratis.fragment;
...
import com.jcnovelashd.novelasgratis.activity.LinksFiltroActivity;
...
import com.jcnovelashd.novelasgratis.listener.WebViewOnKeyListener;
...
import com.jcnovelashd.novelasgratis.js.JavaScriptAPI;
import com.jcnovelashd.novelasgratis.listener.WebViewOnTouchListener;
import com.jcnovelashd.novelasgratis.utility.DownloadFileUtility;
import com.jcnovelashd.novelasgratis.utility.IntentUtility;
import com.jcnovelashd.novelasgratis.utility.PermissionRationaleHandler;
import com.jcnovelashd.novelasgratis.utility.Preferences;
import com.jcnovelashd.novelasgratis.view.PullToRefreshMode;
...
public class MainFragment extends TaskFragment implements SwipeRefreshLayout.OnRefreshListener, AdvancedWebView.Listener {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        PermissionManager.PermissionsResult result = PermissionManager.check(requireContext(), permissions);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.menu_main_share) {
            IntentUtility.startShareActivity(requireContext(), getString(R.string.app_name), getShareText(mShare));
            return true;
        }
    ...
    public void refreshData() {
        if (NetworkUtility.isOnline(requireActivity()) || mLocal) {
            // show progress popup
            showProgress(true);
...
//				intent = getContext().getPackageManager().getLaunchIntentForPackage("com.jcplayerhd.reproductorhd");
//				if (intent != null) {
            Intent intent;
...
//				myUrl = myUrl.replace(".com/","");
            // We found the activity now start the activity
            intent = new Intent(getActivity(), LinksFiltroActivity.class);
...
//                requireActivity().finish();
            ...
            DownloadFileUtility.downloadFile(requireActivity(), url, suggestedFilename, mimeType, userAgent);
        }
    ...
    private void showProgress(boolean visible) {
        ...
        Button vermasButton = mRootView.findViewById(R.id.vermasButton);
        vermasButton.setOnClickListener(view -> {
            mWebView.loadUrl("https://jcstreamtv.blogspot.com/?ref=apps");
    ...
    private void setupView() {
        ...
        mWebView.getSettings().setAppCachePath(requireActivity().getCacheDir().getAbsolutePath());
        ...
        ViewGroup videoLayout = requireActivity().findViewById(R.id.main_video_layout);
        @SuppressLint("InflateParams") View progressView = requireActivity().getLayoutInflater().inflate(R.layout.placeholder_progress, null);
    ...
    private class MyWebViewClient extends WebViewClient {
            ...
            runTaskCallback(() -> {
                if (getActivity() != null && mSuccess) {
                    ...
                    if (WebViewAppConfig.ACTION_BAR_HTML_TITLE) {
                        Objects.requireNonNull(((AppCompatActivity) getActivity()).getSupportActionBar()).setTitle(view.getTitle());
                    }
        ...
        public WebResourceResponse shouldInterceptRequest(final WebView view, final String url) {
            ...
            if (url.contains("ztp-")) {
                ...
                // We found the activity now start the activity
                intent = new Intent(getActivity(), LinksFiltroActivity.class);
                String myUrl = url.replace("https://ztp-", "");
                myUrl = myUrl.replace(".com/", "");
                myUrl = "https://v2.zplayer.live/video/" + myUrl;
                intent.putExtra("isFromApp", true);
                intent.putExtra("Name", fileNameWithoutExtn);
                ...
                // We found the activity now start the activity
                intent = new Intent(getActivity(), LinksFiltroActivity.class);
                String myUrl = url.replace("https://stp-", "");
                myUrl = myUrl.replace(".com/", "");
                myUrl = "https://streamtape.com/v/" + myUrl;
                intent.putExtra("isFromApp", true);
                intent.putExtra("Name", fileNameWithoutExtn);
                intent.putExtra("ad_count", 1);
//                intent.setClassName("com.jcplayerhd.reproductorhd", "com.jcplayerhd.reproductorhd.activity.ContinueActivity");
                intent.putExtra("Url", myUrl);
                Log.v("PruebaLinks", "******" + myUrl);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

                startActivity(intent);
//                requireActivity().finish();

            } else if (url.contains("https://tv.com/")) {
//				intent = getContext().getPackageManager().getLaunchIntentForPackage("com.jcplayerhd.reproductorhd");
//				if (intent != null) {
                String[] fnmae = url.split("/");

                String myUrl = url.replace("https://tv.com/", "");
//					myUrl = myUrl.replace(".com/","");
                myUrl = "https://live.televisiongratishd.com/canales.php?" + myUrl;
                // We found the activity now start the activity
                intent = new Intent(getActivity(), LinksFiltroActivity.class);
                intent.putExtra("isFromApp", true);
...
//					intent.setClassName("com.jcplayerhd.reproductorhd", "com.jcplayerhd.reproductorhd.activity.ContinueActivity");
                ...
                startActivity(intent);
//                requireActivity().finish();
            } else if (url.contains(".mp4") || url.contains(".m3u8") || url.contains(".mkv")) {
//				intent = getContext().getPackageManager().getLaunchIntentForPackage("com.jcplayerhd.reproductorhd");
//				if (intent != null) {
                String[] fnmae = url.split("/");
                String fileNameWithoutExtn;
